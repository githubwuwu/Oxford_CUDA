...
__global__ void reduction_2(float *g_odata, float *g_idata)
{
	//Ê¹ÓÃshuffle
	int tidGlobal = threadIdx.x + blockDim.x * blockIdx.x;
	int tidLocal = threadIdx.x;
	//int tid = threadIdx.x;
	int temp = g_idata[tidGlobal];
	__syncthreads();
	for (int offset = 16; offset > 0; offset /2) 
	{

		//temp += __shfl_down(temp, offset);  Ê§°Ü£¿£¿£¿
	}
...
int main(int argc, const char** argv)
{
	...
	//reduction_2 << <num_elements / warp_size, num_threads, shared_mem_size >> >(d_odata, d_idata);
